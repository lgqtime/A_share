...
# 已打开页面的控件值会保留在会话中。版本迁移仅替换此前的默认值，
# 不覆盖用户主动调整后的区间或勾选状态。
SCREENING_DEFAULTS_VERSION_STATE_KEY = "szse_quant_filter_defaults_version"
SCREENING_DEFAULTS_VERSION = 5
LEGACY_SCREENING_DEFAULT_RANGES: dict[
    str, tuple[tuple[float, float], ...]
] = {
    "szse_quant_filter_rsi_range": ((45.0, 75.0), (47.5, 61.6), (49.1, 62.6)),
    "szse_quant_filter_turnover_range": ((5.0, 10.0), (5.2, 10.8), (5.4, 10.7), (5.6, 10.7)),
...
LEGACY_SCREENING_DEFAULT_BOOLEANS: dict[str, bool] = {
...
# result must not replace the user's selection or risk-filter settings.
...
def apply_optimized_parameter_overrides(
    ...
    """Overlay only optimizer-owned parameters, preserving the other controls."""
...
def _initialize_screening_widget_state() -> None:
    """初始化控件，并将旧的默认会话值迁移到当前默认配置。"""

    for key, value in SCREENING_WIDGET_DEFAULTS.items():
        st.session_state.setdefault(key, deepcopy(value))
    _migrate_legacy_screening_defaults()


def _migrate_legacy_screening_defaults() -> None:
    """只升级仍等于历史默认值的预测页控件。"""

    if (
        st.session_state.get(SCREENING_DEFAULTS_VERSION_STATE_KEY)
        == SCREENING_DEFAULTS_VERSION
    ):
        return

    for key, legacy_values in LEGACY_SCREENING_DEFAULT_RANGES.items():
        current_value = st.session_state.get(key)
        if (
            isinstance(current_value, (tuple, list))
            and tuple(current_value) in legacy_values
        ):
            st.session_state[key] = deepcopy(SCREENING_WIDGET_DEFAULTS[key])

    for key, legacy_value in LEGACY_SCREENING_DEFAULT_BOOLEANS.items():
        if st.session_state.get(key) == legacy_value:
            st.session_state[key] = bool(SCREENING_WIDGET_DEFAULTS[key])

    st.session_state[SCREENING_DEFAULTS_VERSION_STATE_KEY] = SCREENING_DEFAULTS_VERSION
